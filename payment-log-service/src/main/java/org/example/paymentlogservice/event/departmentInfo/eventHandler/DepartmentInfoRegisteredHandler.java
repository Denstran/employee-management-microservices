...
@Component
@Slf4j
public class DepartmentInfoRegisteredHandler implements EventHandler {
    private static final String DEPARTMENT_INFO_REGISTERED = "DepartmentInfoRegistered";
    private final ObjectMapper objectMapper;
    private final DepartmentInfoPaymentLogService paymentLogService;

    @Autowired
    public DepartmentInfoRegisteredHandler(ObjectMapper objectMapper,
    ...
    public void handleEvent(String eventType, String event) {
        if (DEPARTMENT_INFO_REGISTERED.equals(eventType)) return;

        log.info("Handling event: {}, expected event: {}", eventType, DEPARTMENT_INFO_REGISTERED);

        try {
            DepartmentInfoRegistered departmentInfoEvent =
                    objectMapper.readValue(event, DepartmentInfoRegistered.class);
            ...
            throw new RuntimeException("Can't read event: " + event + " for DepartmentInfoRegistered class");
        }
    }
}
